...
namespace com.utkaka.Psd.ImageProcessing.Decoding.DecodeColor {
	[BurstCompile]
	public struct JobDecodeGrayscale : IJobParallelFor {
		...
		public JobDecodeGrayscale(DecoderChannelsData channels, DecoderAlphaData alphaData, ImageLayerData layerData, DecoderByteData byteData, NativeArray<Color32> pixels) {
			_channels = channels;
		...
		public void Execute(int index) {
			...
			var level = _channels.Channel0[srcIndex];
			_pixels[_layerData.GetIndexInvertedByY(layerPixelX, layerPixelY)] = new Color32(level, level, level, alpha);
		}
	}
}